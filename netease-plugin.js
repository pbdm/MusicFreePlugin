...
module.exports = {
    ...
    version: '0.3.8',
    author: 'pbdm',
    description: '最终修复版：完全回退 0.2.4 播放逻辑，保留自建歌单与排行榜',
    supportedSearchType: ['music', 'album', 'artist', 'sheet'],
    userVariables: [{ key: 'cookie', name: 'Cookie', hint: '请输入包含 MUSIC_U 的 Cookie' }],
    ...
    async getRecommendMusic() {
        ...
        if (cookie) {
            result.push({
                title: '账户专属',
                data: [{ id: 'daily_recommend', title: '每日歌曲推荐', artwork: 'https://p1.music.126.net/06vU_T-7_W260uK7mX9U-A==/109951165671182690.jpg', description: '根据口味每天6:00更新' }]
            });
            try {
                ...
                if (uid) {
                    ...
                    const list = res.data.playlist || res.data.playlists || [];
                    if (list.length > 0) {
                        result.push({ title: '我的歌单', data: list.map(p => ({ id: String(p.id), title: p.name, artist: p.creator?.nickname || '', artwork: p.coverImgUrl })) });
                    }
                }
            } catch (e) {}
        }
        try {
            const hotRes = await axios.get('https://music.163.com/api/playlist/list', { params: { cat: '全部', order: 'hot', limit: 18 }, headers: commonHeaders });
            if (hotRes.data && hotRes.data.playlists) {
                result.push({ title: '热门歌单', data: hotRes.data.playlists.map(p => ({ id: String(p.id), title: p.name, artist: p.creator.nickname, artwork: p.coverImgUrl })) });
            }
        } catch (e) {}
        return result;
    },
    ...
    async getMusicSheetInfo(sheetItem, page) {
        ...
        const res2 = await axios.get('https://music.163.com/api/v3/song/detail', { params: { c: JSON.stringify(ids.map(id => ({ id }))), ids: JSON.stringify(ids) }, headers: { ...commonHeaders, Cookie: cookie } });
        return { isEnd: start + ids.length >= playlist.trackIds.length, musicList: (res2.data.songs || []).map(mapMusicItem), sheetItem: page === 1 ? { title: playlist.name, artwork: playlist.coverImgUrl, description: playlist.description } : undefined };
    },
    ...
    async getMusicComments(musicItem, page) {
        const cookie = (env.getUserVariables() || {}).cookie || '';
        const res = await postEapi('https://interface3.music.163.com/eapi/v2/resource/comments', '/api/v2/resource/comments', { threadId: "R_SO_4_" + musicItem.id, cursor: String((page - 1) * 20), sortType: "1", pageNo: page, pageSize: "20", parentCommentId: "0", showInner: false }, cookie);
        const data = res.data || res;
        const comments = (data.comments || []).map(c => ({ id: String(c.commentId), content: c.content, nickName: c.user.nickname, avatar: c.user.avatarUrl, createAt: c.time, like: c.likedCount, location: c.ipLocation?.location }));
        return { isEnd: data.hasMore === false, data: comments };
    },

    // 0.2.4 原始 getMediaSource (禁止修改)
    ...
    async getRecommendSheetTags() {
        ...
        return {
            pinned: [{ id: 'daily_recommend', title: '每日推荐' }, { id: 'user_created', title: '我创建的' }, { id: 'user_collected', title: '我收藏的' }],
    ...
    async getRecommendSheetsByTag(tag, page) {
        ...
        if (tagId === 'user_created' || tagId === 'user_collected' || tagTitle === '我创建的' || tagTitle === '我收藏的') {
            ...
            try {
                ...
                return { isEnd: true, data: list.filter(p => isCreated ? (p.subscribed === false) : (p.subscribed === true)).map(p => ({ id: String(p.id), title: p.name, artist: p.creator?.nickname || '', artwork: p.coverImgUrl })) };
    ...
    async getTopLists() {
        try {
            ...
            return [{ title: '官方榜', data: (res.data.list || []).map(item => ({ id: String(item.id), title: item.name, artwork: item.coverImgUrl, description: item.updateFrequency })) }];
        } catch (e) { return []; }
    },

    async getTopListDetail(topListItem, page) { return this.getMusicSheetInfo(topListItem, page); },
    ...
    async getUserSheet(userId, page) {
        ...
        return { sheetList: (res.data.playlist || []).map(p => ({ id: String(p.id), title: p.name, artist: p.creator?.nickname || '', artwork: p.coverImgUrl, description: p.description })) };
    }
};
